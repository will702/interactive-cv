...
export function projectPoints(
  ...
  return points3D.map(pt => {
    ...
    // If zImg is very close to 0 or behind camera, it might project weirdly,
    // but we'll return it anyway so the caller can clip it if needed.
    return {
      x: xImg / zImg,
      y: yImg / zImg,
      z: zCam // return zCam to help with depth clipping (don't draw points behind camera)
    };
  });
}
